...
def get_file_extension(path):
    """
    Get the file extension, handling .gz compressed files.

    Args:
        path: File path (may end with .gz)

    Returns:
        tuple: (base_extension, is_compressed)
            - base_extension: The actual file extension (e.g., 'pdb', 'cif', 'mrc', 'map')
            - is_compressed: True if file is gzip compressed
    """
    if path.endswith(".gz"):
        # Strip .gz and get the real extension
        base_path = path[:-3]
        ext = base_path.split(".")[-1].lower()
        return ext, True
    else:
...
def open_file(path, mode="r"):
    ...
    Args:
        path: File path (may end with .gz)
        mode: File open mode (e.g., 'r', 'w', 'rb', 'wb')

    Returns:
        File object (regular or gzip depending on extension)
    """
    _, is_compressed = get_file_extension(path)
    if is_compressed:
        # Add 'b' to mode if not present for gzip
        if "b" not in mode:
            mode = mode + "b"
...
def euler_to_mtx(ang):
    ...
    mtx_z = torch.stack(
        (cos_c, -sin_c, zero, sin_c, cos_c, zero, zero, zero, one)
    ).reshape(3, 3)
    mtx_y = torch.stack(
        (cos_b, zero, sin_b, zero, one, zero, -sin_b, zero, cos_b)
    ).reshape(3, 3)
    mtx_x = torch.stack(
        (one, zero, zero, zero, cos_a, -sin_a, zero, sin_a, cos_a)
    ).reshape(3, 3)
...
def get_score(ref_map, tgt_map_data, tgt_map_vec, trans):
    ...
    search_non_zero_mask = (
        tgt_map_data[search_pos[:, 0], search_pos[:, 1], search_pos[:, 2]] > 0
    )
    search_non_zero_count = np.count_nonzero(
        np.multiply(target_zero_mask, search_non_zero_mask)
    )

    trimmed_target_vec = ref_map_vec[
        target_pos[:, 0], target_pos[:, 1], target_pos[:, 2]
    ]
    trimmed_search_vec = tgt_map_vec[
