...
def segment_map(input_map, output_map, contour=0):
    ...
    with mrcfile.open(input_map, permissive=True) as mrc:
        ...
        _nx, _ny, _nz, nxs, nys, nzs, _mx, _my, _mz = (
            mrc.header.nx,
            mrc.header.ny,
            mrc.header.nz,
            mrc.header.nxstart,
        ...
        # map to new axes
        shift_start = permute_map_coord_to_pdb(
            [min_x, min_y, min_z],
            int(mrc.header.mapc),
            int(mrc.header.mapr),
            int(mrc.header.maps),
        ...
        (mrc_new.header.origin.x, mrc_new.header.origin.y, mrc_new.header.origin.z) = (
            origin
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Segment a 3D map based on a given contour level."
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0,
        help="Contour level to use for segmentation.",
    )
