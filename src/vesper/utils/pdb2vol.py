...
def get_atom_list_from_cif(cif_file, backbone_only=False):
    """
    Retrieve the coordinates and atom types from a CIF file using MMCIF2Dict.

    Parameters:
    cif_file (str): The path to the CIF file (may be .gz compressed).
    ...
    _, is_compressed = get_file_extension(cif_file)

    if is_compressed:
        # Write to temp file for MMCIF2Dict which expects a filename
        import os
        import tempfile
    ...
    # Get atom coordinates and types from CIF dictionary
    ...
    # Get residue info for backbone filtering
    if backbone_only:
        auth_asym_id = mmcif_dict.get("_atom_site.auth_asym_id", [None] * len(x_list))
        label_seq_id = mmcif_dict.get("_atom_site.label_seq_id", [None] * len(x_list))
        label_atom_id = mmcif_dict.get("_atom_site.label_atom_id", [None] * len(x_list))
    else:
        auth_asym_id = None
        label_seq_id = None
        label_atom_id = None
    ...
    for i in range(len(x_list)):
        ...
        y = float(y_list[i])
        z = float(z_list[i])
        atom_type = type_symbol_list[i]

        if backbone_only:
            # Get current residue info for filtering
            chain = auth_asym_id[i] if auth_asym_id else None
            res_id = label_seq_id[i] if label_seq_id else None
            atom_name = label_atom_id[i] if label_atom_id else None

            # Only keep backbone atoms (CA, C, N)
...
def get_atom_list(pdb_file, backbone_only=False):
    ...
    Parameters:
    pdb_file (str): The path to the PDB or CIF file (may be .gz compressed).
    ...
    tuple: A tuple containing two elements:
        ...
        - list: A list of atom types.
    """
    ext, is_compressed = get_file_extension(pdb_file)

    if ext == "pdb":
        # For .gz files, need to decompress for Biopython
        if is_compressed:
            import os
            import tempfile
            ...
            try:
                ...
                structure = st_parser.get_structure("protein", tmp_path)
            finally:
                os.unlink(tmp_path)
        else:
            st_parser = PDBParser(QUIET=True)
        ...
        # Use MMCIF2Dict to avoid B-factor dependency issues
        return get_atom_list_from_cif(pdb_file, backbone_only)
    else:
...
@njit(fastmath=True, nogil=True)
def make_atom_overlay_map(
    origin, voxel_size, box_size, atom_list, atom_type_list, atom_mass_dict
):
    """
    ...
    for atom, atom_type in zip(atom_list, atom_type_list):
        ...
        if pos:
            ...
            map_data[pos_z_1, pos_y_1, pos_x_1] += (
                (1 - a) * (1 - b) * (1 - c) * atom_mass
            )
...
def pdb2vol(
    ...
    ext, _ = get_file_extension(input_pdb)
    ...
    new_voxel_size = np.array(
        [
            voxel_size[2] * dims[2] / x_s,
            voxel_size[1] * dims[1] / y_s,
            voxel_size[0] * dims[0] / z_s,
        ]
    )

    # print(new_voxel_size)

    map_data = make_atom_overlay_map(
        origin, new_voxel_size, (z_s, y_s, x_s), atoms, types, atom_mass_dict
    )
    ...
    if resolution * sigma_coeff / new_voxel_size[0] >= 1:
        ...
        print(
            "Warning: Blurring will not be performed because the resolution is too high w.r.t. the voxel size."
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Convert a PDB or CIF file to a volumetric map in MRC format."
    )
    ...
    parser.add_argument(
        "-m", "--ref_map", help="Path to a reference map in MRC format.", default=None
    )
    parser.add_argument(
        "-s",
        "--sigma_coeff",
