...
class EMmap:
    ...
    def __init__(self, path, ss_data=None):
        ...
        self._temp_file = None  # Track temp file for .gz cleanup
        if self.ss_data is not None:
            self.ss_data = np.swapaxes(
                self.ss_data, 0, 2
            )  # order is coil, beta, alpha, nucleotides

        # Handle .gz compressed files
        ext, is_compressed = get_file_extension(path)
        if is_compressed:
            # Decompress to temp file for mrcfile
        ...
        if np.all(self.orig == 0):
            ...
            self.orig = self.orig_idx * np.array(
    ...
    def __del__(self):
        """Cleanup temporary file if created for .gz decompression"""
    ...
    def resample_and_vec(self, dreso=16.0):
        ...
        print(
            f"#MAP SUM={density_sum} COUNT={count} AVE={ave} STD={std} STD_norm={std_norm_ave}"
        )
        if self.ss_data is not None:
        ...
        for y in range(new_dim):
            for z in range(new_dim):
                ...
                if ss_data is not None:
                    dest_ss_data[x][y][z][0], _ = calc(
                        stp, endp, pos, ss_data[..., 0], fsiv
                    )
                    dest_ss_data[x][y][z][1], _ = calc(
                        stp, endp, pos, ss_data[..., 1], fsiv
                    )
                    dest_ss_data[x][y][z][2], _ = calc(
                        stp, endp, pos, ss_data[..., 2], fsiv
                    )
                    dest_ss_data[x][y][z][3], _ = calc(
                        stp, endp, pos, ss_data[..., 3], fsiv
                    )

                if np.isclose(
                    dtotal, 0.0
                ):  # check for infinitesimal density due to numerical error
                    continue
