...
def calc_rmsd(search_pdb_path, ref_pdb_path, rot_mtx, trans_vec, verbose=False):
    ...
    if len(ref_atoms_array) != len(sample_atoms_array):
        ...
        if verbose:
            print("Warning: Number of residues in reference and sample structures do not match")
        ...
        while i < len(atom_list_ref) and j < len(atom_list_sample):
            if atom_list_ref[i].get_full_id()[3][1] < atom_list_sample[j].get_full_id()[3][1]:
                i += 1
            elif atom_list_ref[i].get_full_id()[3][1] > atom_list_sample[j].get_full_id()[3][1]:
                j += 1
            else:
    ...
    rmsd = np.sqrt(np.sum((ref_atoms_array - sample_atoms_array) ** 2) / len(ref_atoms_array))

    return rmsd, search_pdb


def calc_rmsd_list(search_pdb_path, ref_pdb_path_list, vesper_output, verbose=False, gen_pdb=False, topn=10):
    ...
    with open(vesper_output) as f:
        ...
        for line in result_line:
            ...
            rot_vec = np.array([float(r_info[0][3:]), float(r_info[1]), float(r_info[2][:-1])])
            trans_vec = np.array([float(t_info[0][3:]), float(t_info[1]), float(t_info[2][:-1])])
            ...
            for ref_pdb_path in ref_pdb_path_list:
                rmsd, search_pdb = calc_rmsd(search_pdb_path, ref_pdb_path, rotation, trans_vec, verbose=verbose)
                if rmsd < min_rmsd:
                    min_rmsd = rmsd

            if gen_pdb:
                pdbio.set_structure(search_pdb)
                pdbio.save("./rmsd_pdb_results/model_" + pathlib.Path(search_pdb_path).stem + "_#" + model_num + ".pdb")
...
if __name__ == "__main__":
    ...
    parser.add_argument("-n", "--topn", help="Top N models to calculate RMSD", type=int, default=10)
    parser.add_argument("-v", "--verbose", help="Print verbose output", action="store_true")
    parser.add_argument("-g", "--gen_pdb", help="Generate PDB files of the RMSD calculations", action="store_true")
    ...
    rmsd = calc_rmsd_list(args.search_pdb, [args.ref_pdb], args.vesper_output, args.verbose, args.gen_pdb, args.topn)
    print("RMSD: " + str(rmsd))
